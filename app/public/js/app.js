...
function setCSStheme(name){
    ...
    document.getElementById("appcsslink").setAttribute("href", `../../css/${theme}/app.css`)
    document.getElementById(`${name}csslink`).setAttribute("href", `../../css/${theme}/${name}.css`) 
}

function getDisplayInformation(){
    let cookieValue = document.cookie
    .split("; ")
    .find((row) => row.startsWith("DisplayInformation"))?.split("=")[1];
    if (cookieValue){
        cookieValue = JSON.parse(decodeURIComponent(cookieValue).split("j:")[1]);
        return cookieValue
    }
}