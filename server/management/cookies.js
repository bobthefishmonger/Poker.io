...
function encrypt(text) {
	...
	let cipher = crypto.createCipheriv(
		"aes-256-cbc",
		Buffer.from(process.env.SessionInfo_Enkey),
		iv
	);
...
function createSessionInfoCookie(res) {
	...
	res.cookie("SessionInfo", data, {
		httpOnly: true,
		expires: createCookieExpiry(21)
	});
	return decrypt(data);
}
...
async function validateSessionInfocookie(req, res) {
	...
	if (req.cookies["SessionInfo"]) {
		...
		try {
			decryptedSessionInfo = JSON.parse(
				decrypt(req.cookies["SessionInfo"])
			);
		...
		SessionInfo = JSON.parse(createSessionInfoCookie(res));
	}
